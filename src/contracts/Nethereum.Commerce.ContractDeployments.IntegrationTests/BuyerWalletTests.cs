...
namespace Nethereum.Commerce.ContractDeployments.IntegrationTests
{
    ...
    public class BuyerWalletTests
    {
        ...
        private readonly CompleteSampleDeploymentFixture _fixture;
        ...
        public BuyerWalletTests(CompleteSampleDeploymentFixture fixture, ITestOutputHelper output)
        {
        ...
        public async void ShouldGetPo()
        {
            ...
            var signature = poAsRequested.GetSignatureBytes(_fixture.Web3);
            await PrepSendFundsToBuyerWalletForPo(_fixture.Web3, poAsRequested);
            
            var txReceiptCreate = await _fixture.Deployment.Buyer.BuyerWalletService.CreatePurchaseOrderRequestAndWaitForReceiptAsync(poAsRequested, signature);
        ...
        public async void ShouldGetPoBySellerAndQuote()
        {
            ...
            var poAsBuilt = (await _fixture.Deployment.Buyer.BuyerWalletService.GetPoByEshopIdAndQuoteQueryAsync(
        ...
        public async void ShouldCreatePoAndTransferFunds()
        {
            ...
            StandardTokenService sts = new StandardTokenService(_fixture.Web3, poAsRequested.CurrencyAddress);
            var totalPoValue = poAsRequested.GetTotalCurrencyValue();
            var web3AddressBalance = await sts.BalanceOfQueryAsync(_fixture.Web3.TransactionManager.Account.Address);
            ...
            var fundingBalanceBefore = await sts.BalanceOfQueryAsync(_fixture.Deployment.Eshop.FundingService.ContractHandler.ContractAddress);
            ...
            var fundingBalanceAfter = await sts.BalanceOfQueryAsync(_fixture.Deployment.Eshop.FundingService.ContractHandler.ContractAddress);
        ...
        public async void ShouldCreatePoAndRetrieveIt()
        {
            ...
            var signature = poAsRequested.GetSignatureBytes(_fixture.Web3);
            await PrepSendFundsToBuyerWalletForPo(_fixture.Web3, poAsRequested);
            var txReceipt = await _fixture.Deployment.Buyer.BuyerWalletService.CreatePurchaseOrderRequestAndWaitForReceiptAsync(poAsRequested, signature);
            ...
            var poAsBuilt = (await _fixture.Deployment.Buyer.BuyerWalletService.GetPoQueryAsync(
            ...
            var block = await _fixture.Web3.Eth.Blocks.GetBlockWithTransactionsByNumber.SendRequestAsync(txReceipt.BlockNumber);
        ...
        public async void ShouldFailToCreatePoWithoutFunding()
        {
            ...
            Func<Task> act = async () => await _fixture.Deployment.Buyer.BuyerWalletService.CreatePurchaseOrderRequestAndWaitForReceiptAsync(poAsRequested, signature);
        ...
        public async void ShouldFailToCreatePoWhenEshopOrSellerIsInactive()
        {
            ...
            var eShop = (await _fixture.Deployment.BusinessPartners.BusinessPartnerStorageService.GetEshopQueryAsync(poAsRequested.EShopId)).EShop;
            eShop.Should().NotBeNull();
            eShop.IsActive = false;
            var eShopSetTx = await _fixture.Deployment.BusinessPartners.BusinessPartnerStorageService.SetEshopRequestAndWaitForReceiptAsync(eShop);
            ...
            eShop.IsActive = true;
            eShopSetTx = await _fixture.Deployment.BusinessPartners.BusinessPartnerStorageService.SetEshopRequestAndWaitForReceiptAsync(eShop);
            eShopSetTx.Status.Value.Should().Be(1);

            // Temporarily make Seller inactive
            var seller = (await _fixture.Deployment.BusinessPartners.BusinessPartnerStorageService.GetSellerQueryAsync(poAsRequested.SellerId)).Seller;
            seller.Should().NotBeNull();
            seller.IsActive = false;
            var sellerSetTx = await _fixture.Deployment.BusinessPartners.BusinessPartnerStorageService.SetSellerRequestAndWaitForReceiptAsync(seller);
            sellerSetTx.Status.Value.Should().Be(1);

            // Attempt to create PO, it should fail
            act = async () => await _fixture.Deployment.Buyer.BuyerWalletService.CreatePurchaseOrderRequestAndWaitForReceiptAsync(poAsRequested, signature);
            ...
            sellerSetTx = await _fixture.Deployment.BusinessPartners.BusinessPartnerStorageService.SetSellerRequestAndWaitForReceiptAsync(seller);
            sellerSetTx.Status.Value.Should().Be(1);
        }
        ...
        public async void ShouldFailToCreatePoWithoutCorrectSigner()
        {
            ...
            var signature = poAsRequested.GetSignatureBytes(_fixture.Web3SecondaryUser);
            await PrepSendFundsToBuyerWalletForPo(_fixture.Web3, poAsRequested);
            Func<Task> act = async () => await _fixture.Deployment.Buyer.BuyerWalletService.CreatePurchaseOrderRequestAndWaitForReceiptAsync(poAsRequested, signature);
        ...
        private async Task<Buyer.Po> CreateBuyerPoAsync(uint quoteId, bool isLargeValue = false)
        {
            return CreatePoForPurchasingContracts(
                buyerUserAddress: _fixture.Web3.TransactionManager.Account.Address.ToLowerInvariant(),
                buyerReceiverAddress: _fixture.Web3.TransactionManager.Account.Address.ToLowerInvariant(),
                buyerWalletAddress: _fixture.Deployment.Buyer.BuyerWalletService.ContractHandler.ContractAddress.ToLowerInvariant(),
                eShopId: _fixture.Deployment.Eshop.EshopId,
                sellerId: _fixture.Deployment.Seller.SellerId,
                currencySymbol: await _fixture.Deployment.MockDaiService.SymbolQueryAsync(),
                currencyAddress: _fixture.Deployment.MockDaiService.ContractHandler.ContractAddress.ToLowerInvariant(),
                quoteId,
        ...
        {
            var po = (await _fixture.Deployment.Buyer.BuyerWalletService.GetPoQueryAsync(eShopId, poNumber)).Po;
